...
(function () {
    ...
    describe('Instantiation', function () {
        ...
        it('Second argument must be of type object', function () {
            ...
            // it's not possible to detect if a node element is a node inside
            // Node.js since everything is an object, maybe i should have used
            // Phantom.js afterall
            if ('exports' in window){ expect(visualHashNode).to.throw(Error); }
            expect(visualHashArray).to.throw(Error);
            expect(visualHashObj).to.not.throw(Error);
            expect(visualHashStr).to.throw(Error);
        });

        it('If no object is given, options must match the defaults', function () {
            var node = document.createElement('input'),
                visualHash = new VisualHash(node);

            expect(visualHash.options).to.be.eql(visualHash.defaults);
        });
